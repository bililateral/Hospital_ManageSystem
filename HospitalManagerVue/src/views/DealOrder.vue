...
export default {
  ...
  methods: {
    ...
    dataPackage() {
      ...
      oTotalPrice = this.checkTotalPrice + this.drugTotalPrice + this.doctorDPrice;
  ...
  created() {
    ...
    this.requestDrug();
    this.requestCheck();
    // 新增：通过dId查询医生信息，获取挂号费dPrice
    request.get("admin/findDoctor", {
      params: { dId: this.dId } // 使用当前登录医生的dId
    }).then(docRes => {
      if (docRes.data.status === 200) {
        this.doctorDPrice = docRes.data.data.dPrice || 0; // 存储挂号费
      }
    });
  },
};
