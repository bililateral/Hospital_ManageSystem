...
public class OrderServiceImpl implements OrderService {
     ...
     * 根据pId查询挂号
     */
    public List<Orders> findOrderByPid(int pId){return this.orderMapper.findOrderByPid(pId);}
    ...
    public Boolean updateOrder(Orders orders) {
        ...
        return this.orderMapper.update(orders, wrapper) > 0;
    }
    /**
     * 根据id设置缴费状态
     */
    ...
    public Boolean updateOrderByAdd(Orders order){
        return (this.orderMapper.updateOrderByAdd(order) == 0);
    }
    /**
     * 判断诊断之后再次购买药物是否已缴费
     */
    public Boolean findTotalPrice(int oId){
        ...
        if (order.getOTotalPrice() != 0.00){
            order.setOPriceState(0);
            return this.orderMapper.updateById(order) > 0;
        }
        return false;
    }
    /**
