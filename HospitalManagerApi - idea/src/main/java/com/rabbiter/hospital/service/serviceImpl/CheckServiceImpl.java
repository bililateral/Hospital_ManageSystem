...
public class CheckServiceImpl implements CheckService {
    ...
    public Boolean addCheck(Checks checks){
        //如果账号已存在则返回false
        if (this.checkMapper.selectById(checks.getChId())!=null)
            return false;
        return this.checkMapper.insert(checks) > 0;
    }
    /**
     * 删除检查信息
     */
    @Override
    public Boolean deleteCheck(int chId) {return this.checkMapper.deleteById(chId) > 0;}
    /**
     * 修改检查信息
     */
    @Override
