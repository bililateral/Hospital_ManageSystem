...
public class BedServiceImpl implements BedService {
    ...
    public Boolean addBed(Bed bed){
        //如果账号已存在则返回false
        if (this.bedMapper.selectById(bed.getBId())!=null )
            return false;
        bed.setBState(0);
        return this.bedMapper.insert(bed) > 0;
    }
    /**
     * 删除床位信息
     */
    ...
    public Boolean deleteBed(int bId) {
        return this.bedMapper.deleteById(bId) > 0;
    }
    /**
     * 清空床位信息
     */
    public Boolean emptyBed(int bId){
        ...
        return this.bedMapper.update(null, wrapper) > 0;
