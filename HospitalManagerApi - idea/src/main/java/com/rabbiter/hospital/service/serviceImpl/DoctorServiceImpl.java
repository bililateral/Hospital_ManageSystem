...
public class DoctorServiceImpl implements DoctorService {
    ...
    public Boolean addDoctor(Doctor doctor) {
        if (this.doctorMapper.selectById(doctor.getdId()) != null)
            return false; // 账号已存在
        //密码加密
        ...
        doctor.setdPeople(0);
        return this.doctorMapper.insert(doctor) > 0;
    }
    ...
    public Boolean deleteDoctor(int dId) {
        ...
        doctor.setdId(dId);
        return this.doctorMapper.deleteById(doctor) > 0;
    }
...
//        List<Doctor> doctors = this.doctorMapper.selectList(queryWrapper);
    ...
    public Boolean updateStar(int dId, Double dStar){
        return this.doctorMapper.updateStar(dId, dStar);
    }
    /**
     * 上传Excel导入数据
     */
