...
public class DoctorController {
    ...
    public void getCaptcha(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        captchaRedisUtil.setCaptcha("doctor:captcha:" + sessionId, code);
     ...
     */
    @PostMapping("/login")
    public ResponseData login(@RequestParam(value = "dId") int dId,
                              @RequestParam(value = "dPassword") String dPassword,
                              @RequestParam("captcha") String captcha,
                              HttpServletRequest request) {
        // 验证码验证
        String sessionId = request.getSession().getId();
        String storedCode = captchaRedisUtil.getCaptcha("doctor:captcha:" + sessionId);

        if (storedCode == null || !storedCode.equalsIgnoreCase(captcha)) {
            return ResponseData.fail("验证码错误或已过期");
        }

        captchaRedisUtil.deleteCaptcha("doctor:captcha:" + sessionId);
    ...
    public ResponseData findPatientById(int pId){return ResponseData.success("返回患者信息成功！", this.patientService.findPatientById(pId));}
    /**
     * 分页根据科室查询所有医生信息
     */
    @RequestMapping("/findDoctorBySectionPage")
    public ResponseData findDoctorBySectionPage(int pageNumber, int size, String query, String arrangeDate, String dSection){return ResponseData.success("根据科室分页查询所有医生信息成功", this.doctorService.findDoctorBySectionPage(pageNumber, size, query, arrangeDate, dSection));}
    /**
     * 用户评价
     */
